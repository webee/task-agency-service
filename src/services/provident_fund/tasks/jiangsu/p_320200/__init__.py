...
LOGIN_URL='http://58.215.195.18:10010/login_person.jsp'
class Task(AbsFetchTask):
    ...
    def _check_login_params(self, params):
        ...
        if params["other"] == "1":
            ...
            assert 'bh2' in params, '缺少用户名'
            assert 'mm2' in params, '缺少密码'
        # other check
        if params["other"] == "1":
            用户名 = params['bh1']
        elif params["other"] == "2":
    ...
    def _unit_login(self, params: dict):
        ...
        raise AskForParamsError([
            dict(key='other',
                 name='[{"tabName":"公积金账号","tabCode":"2","isEnable":"1"},{"tabName":"身份证号码","tabCode":"1","isEnable":"1"}]',
                 cls='tab', value=params.get('类型Code', '')),
            dict(key='bh1', name='身份证号码', cls='input', tabCode="1", value=params.get('用户名', '')),
            dict(key='mm1', name='密码', cls='input:password', tabCode="1", value=params.get('密码', '')),
            dict(key='bh2', name='公积金账号', cls='input', tabCode="2", value=params.get('用户名', '')),
            dict(key='mm2', name='密码', cls='input:password', tabCode="2", value=params.get('密码', '')),
        ], err_msg)
